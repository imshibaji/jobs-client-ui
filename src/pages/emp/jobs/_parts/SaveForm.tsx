import { getAllCookies, getCookie } from "@/utils/CookieStore";
...
export default function SaveForm({token, jobId, user}: {token: string, jobId?: string | number, user?: User}) {
    ...
    const onInit = () => {
        ...
        .then((data: Company[]) => {
            const filteredData = data.filter((company: Company) => company.userId === user?.id);
        ...
        <div className="flex justify-between items-center mb-4">
            {
                ...
                : <h2 className="text-3xl font-semibold text-purple-700">Post A Job</h2>
            }
            <a href="/emp/jobs" className="text-purple-600 focus:ring-2 focus:outline-none focus:ring-purple-300 font-bold rounded-lg text-sm px-5 py-1.5 text-center border border-purple-600 hover:bg-purple-600 hover:text-white">Back To Jobs</a>
        </div>
