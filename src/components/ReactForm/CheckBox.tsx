...
export default function CheckBox({className, inputClass, labelClass, label, ...props}: Props) {
    ...
    return (
        <div className={className || "w-full"}>
            <div className="flex items-center gap-2">
                <input {...props} type={ props.type || "checkbox"} id={props.id || id} className={(inputClass || "checked:bg-violet-600 checked:border-violet-600 appearance-none w-5 h-5 border-2 border-slate-300 bg-white checked:bg-brand-blue checked:border-brand-blue transition") + (props.error ? " border-red-500" : "")} />
