...
export default function FileUploader({token, onSubmitSuccess, ...props}: FileUploaderProps) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const customFileName = props.customName || 'file-' + sanitizedName + '-' + Date.now(); // Ensure uniqueness
    ...
    return (
        <div>
            <div className="flex flex-row">
                ...
                <button type="button" onClick={handleSubmit} className={ props.buttonClassName || "btn btn-primary ml-2"} disabled={!file}>Upload</button>
            </div>
        </div>
    );
}